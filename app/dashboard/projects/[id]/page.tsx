...
import { ArrowLeft, Edit, Users, Calendar, CheckCircle, Clock, AlertCircle, Plus, User, Crown, Shield, Trash2, UserMinus, Undo2, Redo2, UserRoundMinus, Download } from "lucide-react"
...
export default function ProjectDetailPage() {
  ...
  const hasIncompleteTasks = () => {
    if (!project || project.tasks.length === 0) return false

    return project.tasks.some(task => task.status === 'TODO' || task.status === 'IN_PROGRESS' || task.status === 'REVIEW')
  }
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 flex flex-col sm:flex-row flex-wrap gap-3">
              ...
              {canEditProject(currentUserRole) && (
                <>
                  ...
                  {/* Download Report Button - Next to Edit for Members */}
                  {currentUserRole === 'member' && (
                    <Button
                      onClick={() => window.open(`/api/projects/${project.id}/report`, '_blank')}
                      className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-[8px] text-sm sm:text-base w-full sm:w-auto"
                      title={t('download project report')}
                    >
              ...
              {canAssignTasks(currentUserRole) && (
                <>
                  ...
                  {/* Download Report Button - Next to Create Task for Admins */}
              ...
              {canDeleteProject(currentUserRole) && (
                <>
                  <Button
                    ...
                    disabled={project.status !== 'COMPLETED' && hasIncompleteTasks()}
                    ...
                    title={hasIncompleteTasks() ? t('cannot_delete_project_with_incomplete_tasks') : t('delete project')}
                  >
                    <Trash2 className="w-4 h-4 mr-2 text-white" />
                    {t('delete project')}
                  </Button>
                  {/* Download Report Button - Next to Delete for Founders */}
