...
export default function EditProjectPage() {
  ...
  const areAllTasksCompleted = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const projectWithTasks = data.project;
        
        // If project has no tasks, prevent deletion
        if (!projectWithTasks.tasks || projectWithTasks.tasks.length === 0) {
          return false;
        }
        
        // Check if all tasks are completed
        return projectWithTasks.tasks.every((task: any) => task.status === 'COMPLETED');
      }
    } catch (error) {
  ...
  const handleDeleteProject = async () => {
    ...
    setIsDeleting(true)
    try {
      // Check if all tasks are completed before deletion
      const allTasksCompleted = await areAllTasksCompleted();
      
      if (!allTasksCompleted) {
        addToast({ 
          type: 'error', 
          title: 'Gagal', 
          message: 'Project hanya dapat dihapus jika semua tugas telah selesai' 
        });
        setIsDeleting(false);
