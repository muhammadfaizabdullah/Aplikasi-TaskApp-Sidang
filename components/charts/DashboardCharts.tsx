...
export const DashboardCharts: React.FC<DashboardChartsProps> = ({
  ...
  const performanceOverviewOptions = {
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      legend: {
  ...
  // KPI Score Gauge (using Doughnut)
  const kpiScore = currentMonth.totalTasks > 0 
    ? Math.round((currentMonth.completedTasks / currentMonth.totalTasks) * 100) 
    : 0

  const kpiScoreData = {
    labels: ['KPI Score', 'Remaining'],
    datasets: [
      {
        data: [kpiScore, 100 - kpiScore],
        backgroundColor: [
          'rgba(34, 197, 94, 0.8)',
  ...
  return (
    <div className="space-y-8">
      {/* Top Row - Executive Summary and Performance Overview */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Executive Summary Bar Chart */}
        <div className="bg-white rounded-[8px] shadow-lg border border-gray-100 p-6">
          <div className="h-80">
            <Bar data={executiveSummaryData} options={executiveSummaryOptions} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-[8px] shadow-lg border border-gray-100 p-6">
          <div className="h-80">
            <Doughnut data={performanceOverviewData} options={performanceOverviewOptions} />
          </div>
        </div>
      </div>

      {/* Middle Row - Monthly Trend and KPI Score */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Monthly Trend Line Chart */}
        <div className="bg-white rounded-[8px] shadow-lg border border-gray-100 p-6">
          <div className="h-80">
        ...
        <div className="bg-white rounded-[8px] shadow-lg border border-gray-100 p-6">
          <div className="h-80 relative">
            <Doughnut data={kpiScoreData} options={kpiScoreOptions} />
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <div className="text-4xl font-bold text-gray-900">{kpiScore}%</div>
